...
main()
{
	...
	*/ 
	...
	/* EvaluatePRCC(lhcMatrix, pArray, K, N, outcomeMatrix1, "CumulativePRCC");
...
static void PrintOutTimeArray(double **timeMatrix, int nTimes, matrixDB **lhcMatrix, int sample, double **outcomeMatrix)/*, double **outcomeMatrix1, double **outcomeMatrix2)*/
{
	...
	FILE* outfile3;
	int j, box;
	double deaths, deathperHunK,tot;
	double deathsV, deathsU, Cu, Et, ADd;
	...
	for(j=0; j< nTimes; j+=1000){
		deaths = timeMatrix[j][9]*lhcMatrix[2][sample].value; // A*muA
		if (deaths < 0)  deaths = 0;  // avoid negative death rate
		// total population
		tot = 0;
		for (box=0; box<10; box++) tot += timeMatrix[j][box];
	...
	outfile = fopen("CumAIDSdeathsWVaccine", "a");
	if(outfile == NULL) Error("Cannot open 'CumAIDSdeathsWVaccine' for writing");
	outfile2 = fopen("CumAIDSdeathsNVaccine", "a");
	if(outfile2 == NULL) Error("Cannot open 'CumAIDSdeathsNVaccine' for writing");
	outfile3 = fopen("CumDeathsPrevented", "a");
	if(outfile3 == NULL) Error("Cannot open 'CumDeathsPrevented' for writing");
	
	fprintf(outfile, "CumDeathsWVaccine: ");
	fprintf(outfile2, "CumDeathsNVaccine: ");
	fprintf(outfile3, "CumDeathsPrevented: ");
	
	for(j=0; j< nTimes; j+=1000){
			deathsV = timeMatrix[j][10];
			deathsU = GetCu(lhcMatrix, j, sample);
			fprintf(outfile, "%g ", deathsV);
			fprintf(outfile2, "%g ", deathsU);
			fprintf(outfile3, "%g ", deathsU-deathsV);
			// outcomeMatrix1[sample][j] = deaths;
	}
	fprintf(outfile, "\n");
	fclose(outfile);
	fprintf(outfile2, "\n");
...
static void FillInitialConditions(int sample, matrixDB **lhcMatrix, double *firstrow)
{
	double p, betaW, mu, Fw, VwA, VwU, Vw, Ro, muA, prev, pop;
	...
	// VwU is calculated from Vw and VwA
	VwU = log(0.5)*VwA*Vw / (Vw*log(0.5) + VwA);
	...
	firstrow[7] = 0; 		// YwT
	firstrow[8] = 0; 		// YvwT
	firstrow[9] = firstrow[4]*VwU/(mu + muA); 	// A
	firstrow[10] = 0; // ADd
...
static void GetDerivatives(double *array, double *derivArray, int sample, matrixDB **lhcMatrix)
{
	double X, Yv1, Yv2, Yw1, Yw2, Yvw, YvT, YwT, YvwT, A;
	double dxdt, dyv1dt, dyv2dt, dyw1dt, dyw2dt, dyvwdt;
	double dywtdt, dyvtdt,dyvwtdt, dadt;
	double BwU, BwT, Bv1, Bv2, BvT, BvwU, BvwT, LaV, LaW;
	double Vv, VvU, VvT, VwA, Vw, VwU, VwT, VvwU, VvwT, Fw, Fv, Fvw;
	double phi, ADd, dADddt, TOTAL, p, mu, muA;
	...
	/* progression to AIDS for vaccinated people : 5% in 25 years
	// ln1_perc = log (0.95);   // ln (1 - 5%)
	// Vv = (-1)*ln1_perc * VvU/(25*VvU + ln1_perc);
	Vv = (-1)*ln1_perc * Vw/(25*Vw - 1);
	VvTmp = lhcMatrix[10][sample].value;
	Vv = VvTmp * VvU/(VvU - VvTmp);  // T=T1+T2
	lhcMatrix[10][sample].value = Vv;  */
	
	Vw = lhcMatrix[15][sample].value;
	// VwU = lhcMatrix[16][sample].value;
	VwU = log(0.5)*VwA*Vw / (Vw*log(0.5) + VwA);
	lhcMatrix[16][sample].value = VwU;
	VwT = lhcMatrix[17][sample].value;
	VwT = log(0.5)*VwT*Vw / (Vw*log(0.5) + VwT);
	lhcMatrix[16][sample].value = VwT;
	...
	TOTAL = X + Yv1 + Yv2 + Yw1 + Yw2 + Yvw + YvT + YwT + YvwT;
...
static void InvertMatrix(double **symMatrix, double **invMatrix, int size)
{
	/* hayley's code */
 	int r, c, k;
    double tem, tem2, tem3, temp[200][200];
    FILE *f2;
